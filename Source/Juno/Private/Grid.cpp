...
TArray<const FTile*> FGrid::GetTileNeighbors(
    const uint32 InRow,
...
TMap<const FTile*, const FTile*> FGrid::CalculatePathUsingAStar(
    const FTile* InStart,
    const FTile* InEnd,
    TFunction<float(const FTile&, const FTile&)> InDistanceHeuristic) const
{
...
TArray<const FTile*> FGrid::BacktrackFromEndToStart(
    const FTile& InEnd,
...
float FGrid::CalculateManhattanDistance(
    const FTile& A,
    const FTile& B)
{
...
float FGrid::CalculateEuclideanDistance(
    const FTile& A,
    const FTile& B)
{
